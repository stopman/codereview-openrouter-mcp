...
Pick your reviewer per-request: **GPT-5.5 Pro**, **GPT-5.3 Codex**, **Claude Fable 5**, or **Claude Opus 4.8**. Compare opinions. Get a second (or third) opinion on your code before it ships.
...
- **You pick the reviewer** per-request — compare what GPT-5.5 Pro thinks vs Claude vs GPT-5.3
...
> "Use codereview to review my working diff with GPT-5.5 Pro"
...
> "Review my changes with both GPT-5.5 Pro and Claude and compare what they find"
...
```
review_diff(repo_path=".", model="gptpro", focus="all", context_files=["ARCHITECTURE.md"])
```
...
```
review_plan(plan="We plan to...", codebase_context="", model="gptpro", repo_path=".", context_files=["docs/roadmap.md"])
```
...
```
review_oracle(plan="We plan to...", codebase_context="", model="gptpro")
```
...
| Name | Model | Best for |
|---|---|---|
| `gptpro` | OpenAI GPT-5.5 Pro | Deep-reasoning architect reviews (default) |
...
| `all` | Panel: GPT-5.5 Pro + GPT-5.3 + Claude Fable 5 + Claude Opus 4.8 | Multi-perspective review |

Pass `model="gptpro"`, `model="openai"`, `model="claude"`, `model="opus"`, or `model="all"` to any tool. Default is `gptpro`.
...
> "Review the last commit with both GPT-5.5 Pro and Claude"

Different models catch different things — GPT-5.5 Pro might flag a performance issue that GPT-5.3 misses, while Claude spots an architectural concern neither caught.
...
| `DEFAULT_MODEL` | No | `gptpro` | Default model when none specified |
...
| `OPENROUTER_ZDR` | No | `true` | Route only to Zero-Data-Retention provider endpoints. Set `false` if a model has no ZDR endpoint and routing fails. The `claude` (Fable 5) and `gptpro` (GPT-5.5 Pro) slots have no ZDR endpoint yet and are individually exempted in `MODEL_EXTRA_BODY`. `data_collection: "deny"` (no training on your data) is always enforced regardless |
...
AI Assistant  -->  MCP Server  -->  detect-secrets (redact)  -->  OpenRouter API  -->  LLM(s)
                   (6 tools)        (scan & redact secrets)       (unified routing)    (GPT/Claude/
                       |                                                                Opus)
                    git ops
               (diff/show/branch)
```
